...
if (($_SERVER['REQUEST_METHOD'] === 'POST') and ( !isset($_POST['username'], $_POST['password']))) {
  ...
  getenv('MYSQL_DBPASS') ? $db_pass=getenv('MYSQL_DBPASS') : $db_pass="SecretPassword";
...
if (!$failed) {
  ...
  if(preg_match('/\s/', $username)) {
    $error = 'Input Invalid!'; // no whitespaces
    $failed = True;
  }
  if(preg_match('/[\'"]/', $username)) {
    $error = 'Input Invalid!'; // no quotes
    $failed = True;
  }
  if(preg_match('/[\/\\\\]/', $username)) {
    $error = 'Input Invalid!'; // no slashes
    $failed = True;
  }
  if(preg_match('/(and|or|null|where|limit)/i', $username)) {
    $error = 'Input Invalid!'; // no sqli keywords
    $failed = True;
  }
  ...
  if(preg_match('/(group|order|having|limit)/i', $username)) {
    $error = 'Input Invalid!'; //  no sqli select keywords
    $failed = True;
  }
